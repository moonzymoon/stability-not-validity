# -*- coding: utf-8 -*-
"""探针2: 当前种子下 certified vs uncertified 正确率 + 各q的证内错误."""
import os
...
for q in (0.05, 0.1, 0.2, 0.3):
    ...
    for seed in range(5):
        ...
        etas = []
        for mi in range(8):
            rng = np.random.default_rng(stable_seed('e39', seed, 'DPTA-G',
                                                    mi, q))
        ...
        nc += cert.sum()
        nu += (~cert).sum()
    print(f'q={q}: cert n={nc} correct={np.mean(hc):.3f} | '
          f'uncert n={nu} correct={np.mean(hu):.3f} | '
          f'差={np.mean(hc)-np.mean(hu):+.3f}')
