# -*- coding: utf-8 -*-
"""E103: GRAA 二十种子扩展 (e49 口径, seeds 0-19).
解决审稿原因5: 若 20 种子 Wilcoxon p<0.05, "自检验不显著"攻击消失;
若仍不显著, 更紧的估计诚实入文. -> e103_seeds20.json + e103_summary.json"""
import os
...
def run():
    ...
    for gname in ('pcmci_anom', 'true'):
        ...
        summary[gname] = dict(
            ...
            seeds_positive=int((G > P).sum()), n_seeds=N,
        ...
        raw[gname] = dict(seed_means_g=g_m, seed_means_p=p_m,
                          hits_g=[int(x) for x in gh],
                          hits_p=[int(x) for x in ph])
        print(gname, summary[gname], flush=True)
    json.dump(raw, open(os.path.join(CACHE, 'e103_seeds20.json'), 'w'))
    json.dump(summary, open(os.path.join(CACHE, 'e103_summary.json'), 'w'))
    print('-> e103_seeds20.json / e103_summary.json')
