...
# LOO-seed: 同一物理种子 (scm0..4 / rel_scm0..4) 的 dev+rel 全部同侧留出.
# v2 修复: 旧版 'rel_scm0'.split('_')[0].replace('rel','') 解析为空串,
# 导致 relational 全部落入同一"空组"折 --- 感谢独立审核指出。
import re as _re


def seeds_of(ds_list):
    return np.array([_re.search(r'\d+', d).group(0) for d in ds_list])


sd = seeds_of(dsd)
sr = seeds_of(dsr)
assert set(sd) == set(sr), f'seed sets differ: {set(sd)} vs {set(sr)}'
...
for s in sorted(set(sd) | set(sr)):
    ...
    print(f'  fold seed {s}: train dev={int((~trd).sum())} rel='
          f'{int((~trr).sum())} | test dev={int(ted.sum())} '
          f'rel={int(ter.sum())} | '
          f'train-balance={ytr.mean():.2f}')
